...
bConds = 'neumann'; nx = 200; Lx = 50; hx = Lx/(nx-1); x = hx*[0:nx-1]';

%% Get linear operators
L = LinearOperators(x,bConds);

%% Load initial guess and parameters, interpolate on the new grid
sol = load('stableSolution.mat'); u0 = sol.u; p0 = sol.p;
xx =  -Lx+Lx/nx*[0:2*nx-1]';
u0 = interp1(xx, u0, x,'pchip');
